/**
 * Conversion functions for migrating between AI SDK v4 and v5 message formats
 * Based on official AI SDK 5 migration guide
 */
...
export type MyUIMessage = UIMessage<unknown, { custom: unknown }, UITools>;
...
/**
 * Convert AI SDK v4 message to v5 format
 * Used when loading messages from database (v4) to application (v5)
 */
...
/**
 * Convert AI SDK v5 message to v4 format
 * Used when saving messages from application (v5) to database (v4)
 */
...
/**
 * Helper function to extract text content from v5 message parts
 * Useful for display and rendering components
 */
export function extractTextFromV5Message(msg: MyUIMessage): string {
  for (const part of msg.parts) {
    if (part.type === 'text') {
      return part.text;
    }
...
/**
 * Helper function to convert simple content string to v5 parts array
 * Used when creating new messages
 */
export function contentToParts(content: string): MyUIMessage['parts'] {
